...
const legend = Object.values(WAYPOINT_TYPE_META)

const mapRef = useTemplateRef('mapRef')
const highlight = ref<{ lat: number, lon: number } | null>(null)

function onHover(p: TracePoint | null) {
  highlight.value = p ? { lat: p.lat, lon: p.lon } : null
}

function onSelect(p: TracePoint) {
  ...
  <div class="flex flex-col lg:h-[calc(100vh-var(--ui-header-height))]">
    <div class="border-b border-default flex flex-wrap items-center gap-x-4 gap-y-1 px-4 py-2 text-xs">
      ...
      <span class="flex items-center gap-1.5">
        <span
          class="inline-block h-0.5 w-5"
          style="background:repeating-linear-gradient(90deg,#ea580c 0 4px,transparent 4px 8px)"
        /> Variante Incudine
      </span>
      <ClientOnly>
        <UButton
