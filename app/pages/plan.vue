...
import type { PlanDay, PlanNight } from '~/types'

const {
  plan, days, arrivalWaypoints, stopCandidates, nightWaypointIds, toggleStop, initFromOfficial, resetPlan,
...
// décocher une nuitée détruit ses infos de réservation : confirmation si elle en porte
function onToggleStop(waypointId: string) {
  const b = plan.value.nights.find((n) => n.waypointId === waypointId)?.booking
  if (b && (b.status !== 'a_reserver' || b.reference || b.notes || b.prixPayeEur != null)) {
...
function accommodationItems(night: PlanNight) {
  // note Google en tête : les noms sont longs et le menu tronque la fin
...
function formuleItems(night: PlanNight) {
  // `value` élargi en string : le plan conserve une formule même disparue du référentiel (cf. staleChoices)
  return (accommodationFor(night)?.formules ?? []).map((f) => ({
    label: `${FORMULE_LABELS[f.type]} — ${formatPrice(f.prix_eur)}${f.prix_eur != null ? `/${FORMULE_PAR_LABELS[f.par]}` : ''}`,
    value: f.type as string,
  }))
}

const statusItems = Object.entries(BOOKING_STATUS_META).map(([value, m]) => ({ label: m.label, value, icon: m.icon }))
...
function nightDispo(day: PlanDay) {
  const acc = day.accommodation
  if (!acc || !day.date || acc.reservation.canal !== 'pnr-resa' || !dispoFor(acc.id, day.date)) return null
  return { accommodation: acc, dateIso: day.date, selectedFormule: day.night?.formuleType ?? null }
}
...
function confirmRemoveOrphan(waypointId: string, nom: string | null) {
  ...
  const detail = [b?.reference && `réf. ${b.reference}`, b?.prixPayeEur != null && `${b.prixPayeEur} € payés`, b?.notes]
    .filter(Boolean)
    .join(', ')
  if (window.confirm(`Supprimer définitivement la nuitée « ${nom ?? waypointId} »${detail ? ` et ses infos (${detail})` : ''} ?`)) {
    ...
    >
      <template #header>
        <div class="flex items-start gap-2">
          ...
          <div>
            <p class="font-medium">
              {{ orphanNights.length }} nuitée(s) hors tracé
            </p>
      ...
      <ul class="space-y-2 text-sm">
        ...
        >
          ...
          >{{ o.night.waypointId }}</code>
          <UBadge
            :color="BOOKING_STATUS_META[o.night.booking.status].color"
          ...
          />
          <span
            v-if="o.night.booking.reference"
            class="text-muted text-xs"
          >réf. {{ o.night.booking.reference }}</span>
          <span
            v-if="o.night.booking.prixPayeEur != null"
            class="text-muted text-xs tabular-nums"
          >
            {{ o.night.booking.prixPayeEur }} € payés
          </span>
          <span
            v-if="o.night.booking.notes"
            class="text-muted text-xs italic"
          >« {{ o.night.booking.notes }} »</span>
          <UButton
            class="ml-auto"
        ...
        <UCard>
          ...
          <div class="space-y-4">
            <UFormField label="Date de départ (Calenzana)">
              <UInput
                v-model.nullable="plan.startDate"
                type="date"
                min="2020-01-01"
                max="2099-12-31"
                class="w-full"
            ...
            <UFormField
              ...
              help="Sert à estimer ta position pour la météo horaire"
            >
              <UInput
        ...
        <UCard>
          ...
          <ul class="space-y-1.5 text-xs">
            <li
              ...
              :class="d.passed ? 'text-muted line-through' : ''"
            >
              <UIcon
                :name="d.passed ? 'i-lucide-check' : 'i-lucide-bell'"
            ...
            <li
              ...
              class="text-muted"
            >
              Renseigne la date de départ pour voir l'échéancier.
            </li>
          </ul>
        </UCard>
      </div>
        ...
        >
          <div class="flex flex-wrap items-baseline justify-between gap-2">
            <div>
              ...
              <span
                v-if="day.date"
                class="text-muted ml-2 text-sm"
              >{{ formatDateFr(day.date) }}</span>
            </div>
            <div class="text-muted flex flex-wrap items-center gap-3 text-sm tabular-nums">
              <MeteoNight
                v-if="day.date"
                :waypoint-id="day.to.id"
                :date-iso="day.date"
                :altitude="day.to.altitude_m"
              />
          ...
          >
            <UBadge
              ...
              icon="i-lucide-triangle-alert"
              size="sm"
              label="Journée très longue (> 9 h de marche)"
            />
          </div>
          ...
          <template v-if="day.night">
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              >
                <USelect
                  v-model.nullable="day.night.accommodationId"
                  value-key="value"
                  :items="accommodationItems(day.night)"
                  placeholder="Choisir…"
                  class="w-full"
                  @update:model-value="day.night.formuleType = null"
                />
              </UFormField>
              <UFormField
                label="Formule"
              ...
              >
                <USelect
                  v-model.nullable="day.night.formuleType"
                  value-key="value"
              ...
              <UFormField
                label="Référence / n° de résa"
                size="sm"
              >
                <UInput
                  v-model="day.night.booking.reference"
                  placeholder="ex. facture PNRC"
                  class="w-full"
                />
              </UFormField>
            </div>
            ...
            <div class="mt-3 flex flex-wrap items-center gap-3">
              <UBadge
                :color="BOOKING_STATUS_META[day.night.booking.status].color"
              ...
              <DispoNight
                v-if="nightDispo(day)"
                v-bind="nightDispo(day)!"
              />
              <GoogleNote
                v-if="day.night.accommodationId"
                :accommodation-id="day.night.accommodationId"
              />
              <template v-if="day.accommodation">
                <UButton
                  v-if="day.accommodation.reservation.canal === 'pnr-resa'"
                ...
                />
                <UButton
                  v-else-if="day.accommodation.reservation.telephone"
                  icon="i-lucide-phone"
                  size="xs"
                  variant="link"
                  :label="day.accommodation.reservation.telephone"
                  :to="telHref(day.accommodation.reservation.telephone)"
                />
                <span
                  v-if="day.date && day.accommodation.reservation.canal === 'pnr-resa'"
                  class="text-muted text-xs"
                >
              ...
              <div class="ml-auto flex items-center gap-2">
                ...
                <UInputNumber
                  v-model="day.night.booking.prixPayeEur"
                  :min="0"
                  size="xs"
                  class="w-24"
                  placeholder="€"
